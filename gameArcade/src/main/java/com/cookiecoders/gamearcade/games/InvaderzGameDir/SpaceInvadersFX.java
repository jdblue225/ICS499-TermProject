...
import com.cookiecoders.gamearcade.games.InvaderzGame;
...
public class SpaceInvadersFX extends Application {
    ...
    private void spawnEnemies() {
        for (int y = 80, i = 0; y < APP_HEIGHT / 2 + SPACE && i < 5; y += SPACE, i++) {
            for (int x = APP_WIDTH/3 - (SPACE*3), j = 0; x < 660 && j < 13; x += SPACE, j++) {
                if (y < 90) {
                    enemies[i][j] = spawnAlien(x, y, "images/small_invader_a.jpg");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                } else if (y < 200) {
                    enemies[i][j] = spawnAlien(x, y, "images/medium_invader_a.jpg");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                } else {
                    enemies[i][j] = spawnAlien(x, y, "images/large_invader_a.jpg");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                }
                totalEnemies++;
            }
    ...
    private void setMovedEnemies() {
        for (int y = 80, i = 0; y < APP_HEIGHT / 2 + SPACE && i < 5; y += SPACE, i++) {
            for (int x = APP_WIDTH/3 - (SPACE*3), j = 0; x < 660 && j < 13; x += SPACE, j++) {
                if (y < 90) {
                    enemiesMoved[i][j] = spawnAlien(x, y, "images/small_invader_b.jpg");
                } else if (y < 200) {
                    enemiesMoved[i][j] = spawnAlien(x, y, "images/medium_invader_b.jpg");
                } else {
                    enemiesMoved[i][j] = spawnAlien(x, y, "images/large_invader_b.jpg");
                }
            }
        }
    }
    ...
    private void shootMissile() {
        ...
        missile.setImage("images/rocket.jpg");
            ...
            for (int j = 0; j < currentEnemies[0].length; j++) {
                if (currentEnemies[i][j] != null) {
                    if (currentEnemies[i][j].intersects(missiles.get(0))) {
                        ...
                        missiles.clear();
                        MISSILE_LAUNCHED = false; // Reset missile launch status
                        return true;
                    }
                }
            }
    ...
    private void shootPlayer() {
        ...
        alienBomb.setImage("images/missile.jpg");
    ...
    private void setSoundEffects() {
        shootEffect = new SoundEffect("sounds/shoot.mp3");
        killEffect = new SoundEffect("sounds/alienKilled.mp3");
        ufoEffect = new SoundEffect("sounds/ufo.mp3");
        explosionEffect = new SoundEffect("sounds/explosion.mp3");
        setAlienMoveSounds();
    }

    private void setAlienMoveSounds() {
        moveEffects.add(new SoundEffect("sounds/alienMove.mp3"));
        moveEffects.add(new SoundEffect("sounds/alienMove2.mp3"));
        moveEffects.add(new SoundEffect("sounds/alienMove3.mp3"));
        moveEffects.add(new SoundEffect("sounds/alienMove4.mp3"));
    }
